...
def load_data(stage, args, indices=None):
    ...
    if args["DATASET"] == "IMDB":
        ...
        data = pd.read_csv("IMDB_Dataset.csv", header=None)
        data.iloc[:,1] = data.iloc[:,1].apply(lambda x: modify_label(x))
        data = data.drop(data[data.iloc[:,1] == -1].index)
        ...
        raw_data = {
            "review": [example for example in data.iloc[:,0]],
            "sentiment": [example for example in data.iloc[:,1]],
        }
